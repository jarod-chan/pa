...
import cn.fyg.pa.interfaces.web.module.admin.yearchkrpt.PointItem;
...
public class Point implements PointItem {
	...
	//参与度调节系数
	private Participation ptn=new Participation();
	...
	/**
	 * 根据提供的值，计算s 公式如下 [Scheck+(Mall-Mdep)]/Stotal
	 * 
	 * 个人得分 + ( 员工平均得分 - 部门平均分 ) ----------------------------------- 总分
	 */
	public void calculatS() {
		if (damp.compareTo(Constant.ZERO) == 0)
			return;
	...
	public void calculatUpsilon() {
		...
		upsilon = s.subtract(savg).multiply(alpha).multiply(Constant.HUNDRED)
				.setScale(Constant.SCALE, Constant.ROUND_MODEL);
	}

	/**
	 * 计算合计值 Result=upsilon+val+ptn.Is
	 */
	public void calculatResult() {
		val = val.setScale(Constant.SCALE, Constant.ROUND_MODEL);
		ptn.setIs(ptn.getIs().setScale(Constant.SCALE, Constant.ROUND_MODEL));
		result = upsilon.add(val).add(ptn.getIs());
	}
	...
	public Participation getPtn() {
		return ptn;
	}

	public Point ptn(Participation ptn) {
		this.ptn = ptn;
		return this;
	}
